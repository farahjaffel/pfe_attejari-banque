...
package dwh_attejaribank.job_load_dim_type_int_0_1;
...
/**
 * Job: Job_Load_DIM_TYPE_INT Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class Job_Load_DIM_TYPE_INT implements TalendJob {
...
public String getSchema_staging(){
	...
	private final String jobName = "Job_Load_DIM_TYPE_INT";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Job_Load_DIM_TYPE_INT.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Job_Load_DIM_TYPE_INT.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
    final static byte[] commonByteArrayLock_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT = new byte[0];
    static byte[] commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT.length) {
				if(length < 1024 && commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT.length == 0) {
   					commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT = new byte[1024];
				} else {
   					commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT, 0, length);
			strReturn = new String(commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT, 0, length);
			strReturn = new String(commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT) {
...
public static class type_intrStruct implements routines.system.IPersistableRow<type_intrStruct> {
    final static byte[] commonByteArrayLock_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT = new byte[0];
    static byte[] commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_TYPE_INT = new byte[0];
	protected static final int DEFAULT_HASHCODE = 1;
    protected static final int PRIME = 31;
	...
	public boolean equals(Object obj) {
		...
		if (obj == null) return false;
		if (getClass() != obj.getClass()) return false;
		final type_intrStruct other = (type_intrStruct) obj;
	...
	public void copyDataTo(type_intrStruct other) {
	...
	public void copyKeysDataTo(type_intrStruct other) {
     ...
     * Compare keys
     */
    public int compareTo(type_intrStruct other) {
...
type_intrStruct type_intr = new type_intrStruct();
...
if(dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
	tableName_tDBOutput_1 = ("DIM_TYPE_INTR");
} else {
	tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("DIM_TYPE_INTR");
}
...
int count_tDBOutput_1=0;
                                ...
                                try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null, new String[]{"TABLE"})) {
                                    ...
                                    while(rsTable_tDBOutput_1.next()) {
                                        ...
                                        if(table_tDBOutput_1.equals(("DIM_TYPE_INTR"))
                                ...
                                if(!whetherExist_tDBOutput_1) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
                                        stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1 + "\"(\"id_type_intr_sk\" SERIAL  not null ,\"type_interaction\" VARCHAR(18)  ,primary key(\"type_interaction\"))");
                                    }
                                }
	    String update_tDBOutput_1 = "UPDATE \"" + tableName_tDBOutput_1 + "\" SET \"type_interaction\" = ? WHERE \"type_interaction\" = ?";
	    java.sql.PreparedStatement pstmtUpdate_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(update_tDBOutput_1);
	    resourceMap.put("pstmtUpdate_tDBOutput_1", pstmtUpdate_tDBOutput_1);
	    String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1 + "\" (\"type_interaction\") VALUES (?)";
 ...
 */
	...
	currentComponent="tUniqRow_1";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"type_intr");
					}
...
type_intrStruct type_intr_tmp = new type_intrStruct();
// ###############################
...
// # Output table : 'type_intr'
type_intr_tmp.id_type_intr_sk = 0;
type_intr_tmp.type_interaction = row1.type_interaction ;
type_intr = type_intr_tmp;
// ###############################
...
row2 = null;			
if(type_intr.type_interaction == null){
	finder_tUniqRow_1.type_interaction = null;
}else{
	finder_tUniqRow_1.type_interaction = type_intr.type_interaction.toLowerCase();
}	
...
if(type_intr.type_interaction == null){
	new_tUniqRow_1.type_interaction = null;
}else{
	new_tUniqRow_1.type_interaction = type_intr.type_interaction.toLowerCase();
}
		
		keystUniqRow_1.add(new_tUniqRow_1);if(row2 == null){ 
	
	row2 = new row2Struct();
}row2.id_type_intr_sk = type_intr.id_type_intr_sk;			row2.type_interaction = type_intr.type_interaction;					
...
///////////////////////		
				...
				strBuffer_tLogRow_1.append(
				                String.valueOf(row2.id_type_intr_sk)							
				);
	    		...
	    		if(row2.type_interaction != null) { //              
                    							
       
				strBuffer_tLogRow_1.append(
				                String.valueOf(row2.type_interaction)							
				);
	...
	currentComponent="tDBOutput_1";
        ...
        whetherReject_tDBOutput_1 = false;
            int updateFlag_tDBOutput_1=0;
                    if(row3.type_interaction == null) {
pstmtUpdate_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(1, row3.type_interaction);
}


                    if(row3.type_interaction == null) {
pstmtUpdate_tDBOutput_1.setNull(2 + count_tDBOutput_1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(2 + count_tDBOutput_1, row3.type_interaction);
}
...
} else {pstmtInsert_tDBOutput_1.setString(1, row3.type_interaction);
}
...
globalMap.put("tUniqRow_1_NB_DUPLICATES",nb_duplicates_tUniqRow_1);
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"type_intr");
			  	}
 ...
 */
    ...
    public static void main(String[] args){
        final Job_Load_DIM_TYPE_INT Job_Load_DIM_TYPE_INTClass = new Job_Load_DIM_TYPE_INT();

        int exitCode = Job_Load_DIM_TYPE_INTClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Job_Load_DIM_TYPE_INT.class.getClassLoader().getResourceAsStream("dwh_attejaribank/job_load_dim_type_int_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Job_Load_DIM_TYPE_INT.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Job_Load_DIM_TYPE_INT");
        }
...
/************************************************************************************************
 *     95727 characters generated by Talend Open Studio for Data Integration 
 *     on the 16 avril 2026 à 15:49:01 WAT
 ************************************************************************************************/