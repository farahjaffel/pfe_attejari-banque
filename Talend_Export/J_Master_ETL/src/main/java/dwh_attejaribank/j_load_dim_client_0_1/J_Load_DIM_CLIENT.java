...
package dwh_attejaribank.j_load_dim_client_0_1;
...
/**
 * Job: J_Load_DIM_CLIENT Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class J_Load_DIM_CLIENT implements TalendJob {
...
public String getSchema_staging(){
	...
	private final String jobName = "J_Load_DIM_CLIENT";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				J_Load_DIM_CLIENT.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(J_Load_DIM_CLIENT.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class clientStruct implements routines.system.IPersistableRow<clientStruct> {
    final static byte[] commonByteArrayLock_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT = new byte[0];
    static byte[] commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT.length) {
				if(length < 1024 && commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT.length == 0) {
   					commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT = new byte[1024];
				} else {
   					commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT, 0, length);
			strReturn = new String(commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT, 0, length);
			strReturn = new String(commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT) {
...
public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
    ...
    static byte[] commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_CLIENT = new byte[0];
			    ...
			    public Integer enfants_a_charge;

				public Integer getEnfants_a_charge () {
			    ...
			    public Long frais_bancaires_mensuels;

				public Long getFrais_bancaires_mensuels () {
					return this.frais_bancaires_mensuels;
				}
				
			    public Long total_frais_cumules;

				public Long getTotal_frais_cumules () {
					return this.total_frais_cumules;
				}
				
			    public Long revenu_mensuel;

				public Long getRevenu_mensuel () {
					return this.revenu_mensuel;
				}
				
			    public Integer nb_produits_actifs;

				public Integer getNb_produits_actifs () {
        	...
        	try {
        		...
        		int length = 0;
					...
					this.situation_matrimoniale = readString(dis);
						...
						this.enfants_a_charge = readInteger(dis);
					...
					this.methode_paiement_fav = readString(dis);
			            ...
			            length = dis.readByte();
           				if (length == -1) {
           			    	...
           			    	this.frais_bancaires_mensuels = dis.readLong();
           				}
			            ...
			            length = dis.readByte();
           				if (length == -1) {
           			    	...
           			    	this.total_frais_cumules = dis.readLong();
           				}
			            ...
			            length = dis.readByte();
           				if (length == -1) {
           			    	...
           			    	this.revenu_mensuel = dis.readLong();
           				}
					
						this.nb_produits_actifs = readInteger(dis);
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// Integer
						...
						writeInteger(this.enfants_a_charge,dos);
					...
					// Long
						...
						if(this.frais_bancaires_mensuels == null) {
			                dos.writeByte(-1);
               				...
               				dos.writeByte(0);
           			    	dos.writeLong(this.frais_bancaires_mensuels);
		            	}
					...
					// Long
						...
						if(this.total_frais_cumules == null) {
			                dos.writeByte(-1);
               				...
               				dos.writeByte(0);
           			    	dos.writeLong(this.total_frais_cumules);
		            	}
					...
					// Long
						...
						if(this.revenu_mensuel == null) {
			                dos.writeByte(-1);
               				...
               				dos.writeByte(0);
           			    	dos.writeLong(this.revenu_mensuel);
		            	}
					
					// Integer
				
						writeInteger(this.nb_produits_actifs,dos);
...
int count_tDBOutput_1=0;
                                ...
                                try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null, new String[]{"TABLE"})) {
                                    ...
                                    while(rsTable_tDBOutput_1.next()) {
                                        ...
                                        if(table_tDBOutput_1.equals(("DIM_CLIENT"))
                                ...
                                if(!whetherExist_tDBOutput_1) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
                                        stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1 + "\"(\"id_client_sk\" SERIAL  not null ,\"id_client\" VARCHAR(10)   not null ,\"id_produit_fk\" SERIAL  not null ,\"id_profil_fk\" SERIAL  not null ,\"id_financier_fk\" SERIAL  not null ,\"id_digital_fk\" SERIAL  not null ,\"sexe\" VARCHAR(1)  ,\"date_naissance\" DATE ,\"anciennete_mois\" INT8 ,\"score_risque_interne\" INT8 ,primary key(\"id_client\"))");
                                    }
                                }
	    String update_tDBOutput_1 = "UPDATE \"" + tableName_tDBOutput_1 + "\" SET \"id_client\" = ?,\"id_produit_fk\" = ?,\"id_profil_fk\" = ?,\"id_financier_fk\" = ?,\"id_digital_fk\" = ?,\"sexe\" = ?,\"date_naissance\" = ?,\"anciennete_mois\" = ?,\"score_risque_interne\" = ? WHERE \"id_client\" = ?";
	    java.sql.PreparedStatement pstmtUpdate_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(update_tDBOutput_1);
	    resourceMap.put("pstmtUpdate_tDBOutput_1", pstmtUpdate_tDBOutput_1);
	    String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1 + "\" (\"id_client\",\"id_produit_fk\",\"id_profil_fk\",\"id_financier_fk\",\"id_digital_fk\",\"sexe\",\"date_naissance\",\"anciennete_mois\",\"score_risque_interne\") VALUES (?,?,?,?,?,?,?,?,?)";
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = context.password; 
            ...
            row1.enfants_a_charge = rs_tDBInput_1.getInt(6);
            if(rs_tDBInput_1.wasNull()){
                    row1.enfants_a_charge = null;
            }
		                    }
            ...
            row1.frais_bancaires_mensuels = rs_tDBInput_1.getLong(19);
            if(rs_tDBInput_1.wasNull()){
                    row1.frais_bancaires_mensuels = null;
            }
		                    }
            ...
            row1.total_frais_cumules = rs_tDBInput_1.getLong(20);
            if(rs_tDBInput_1.wasNull()){
                    row1.total_frais_cumules = null;
            }
		                    }
            ...
            row1.revenu_mensuel = rs_tDBInput_1.getLong(21);
            if(rs_tDBInput_1.wasNull()){
                    row1.revenu_mensuel = null;
            }
		                    }
            ...
            row1.nb_produits_actifs = rs_tDBInput_1.getInt(22);
            if(rs_tDBInput_1.wasNull()){
                    row1.nb_produits_actifs = null;
            }
		                    }
            ...
            row1.churn = rs_tDBInput_1.getInt(24);
            if(rs_tDBInput_1.wasNull()){
                    row1.churn = null;
            }
		                    }
 					    ...
 					    boolean forceLooprow2 = false;
 							...
 							row2Struct row2ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_1 = false;
                        		    		    ...
                        		    		    row2HashKey.id_client = row1.id_client ;
				           		...
				           		if(tHash_Lookup_row2 != null && tHash_Lookup_row2.getCount(row2HashKey) > 1) { // G 071
									...
									//System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'row2' and it contains more one result from keys :  row2.id_client = '" + row2HashKey.id_client + "'");
								} // G 071
 					    ...
 					    boolean forceLooprow5 = false;
 							...
 							row5Struct row5ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_1 = false;
                        		    		    ...
                        		    		    row5HashKey.id_client = row1.id_client ;
				           		...
				           		if(tHash_Lookup_row5 != null && tHash_Lookup_row5.getCount(row5HashKey) > 1) { // G 071
									...
									//System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'row5' and it contains more one result from keys :  row5.id_client = '" + row5HashKey.id_client + "'");
								} // G 071
...
// # Output table : 'client'
client_tmp.id_client_sk = 0;
client_tmp.id_client = row1.id_client ;
client_tmp.id_produit_fk = row4.id_produit_sk ;
client_tmp.id_profil_fk = row5.id_profil_sk ;
client_tmp.id_financier_fk = row3.id_financier_sk ;
client_tmp.id_digital_fk = row2.id_digital_sk ;
client_tmp.sexe = row1.sexe ;
client_tmp.date_naissance = row1.date_naissance ;
client_tmp.anciennete_mois = row1.anciennete_mois ;
client_tmp.score_risque_interne = row1.score_risque_interne ;
client = client_tmp;
// ###############################
...
} else {pstmtUpdate_tDBOutput_1.setString(1, client.id_client);
}

                    pstmtUpdate_tDBOutput_1.setInt(2, client.id_produit_fk);

                    pstmtUpdate_tDBOutput_1.setInt(3, client.id_profil_fk);

                    pstmtUpdate_tDBOutput_1.setInt(4, client.id_financier_fk);

                    pstmtUpdate_tDBOutput_1.setInt(5, client.id_digital_fk);

                    if(client.sexe == null) {
pstmtUpdate_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(6, client.sexe);
}

                    if(client.date_naissance != null) {
pstmtUpdate_tDBOutput_1.setTimestamp(7, new java.sql.Timestamp(client.date_naissance.getTime()));
} else {
pstmtUpdate_tDBOutput_1.setNull(7, java.sql.Types.TIMESTAMP);
}

                    if(client.anciennete_mois == null) {
pstmtUpdate_tDBOutput_1.setNull(8, java.sql.Types.INTEGER);
} else {pstmtUpdate_tDBOutput_1.setLong(8, client.anciennete_mois);
}

                    if(client.score_risque_interne == null) {
pstmtUpdate_tDBOutput_1.setNull(9, java.sql.Types.INTEGER);
} else {pstmtUpdate_tDBOutput_1.setLong(9, client.score_risque_interne);
}


                    if(client.id_client == null) {
pstmtUpdate_tDBOutput_1.setNull(10 + count_tDBOutput_1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(10 + count_tDBOutput_1, client.id_client);
}
...
} else {pstmtInsert_tDBOutput_1.setString(1, client.id_client);
}

                        pstmtInsert_tDBOutput_1.setInt(2, client.id_produit_fk);

                        pstmtInsert_tDBOutput_1.setInt(3, client.id_profil_fk);

                        pstmtInsert_tDBOutput_1.setInt(4, client.id_financier_fk);

                        pstmtInsert_tDBOutput_1.setInt(5, client.id_digital_fk);

                        if(client.sexe == null) {
pstmtInsert_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(6, client.sexe);
}

                        if(client.date_naissance != null) {
pstmtInsert_tDBOutput_1.setTimestamp(7, new java.sql.Timestamp(client.date_naissance.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_1.setLong(8, client.anciennete_mois);
}

                        if(client.score_risque_interne == null) {
pstmtInsert_tDBOutput_1.setNull(9, java.sql.Types.INTEGER);
} else {pstmtInsert_tDBOutput_1.setLong(9, client.score_risque_interne);
}
...
public static class row2Struct implements routines.system.IPersistableComparableLookupRow<row2Struct> {
    ...
    public String loopKey;
			    ...
			    public int id_digital_sk;

				public int getId_digital_sk () {
					return this.id_digital_sk;
				}
	...
	public void copyDataTo(row2Struct other) {
		...
		other.id_digital_sk = this.id_digital_sk;
	            other.id_client = this.id_client;
	            other.service_sms_banking = this.service_sms_banking;
	            other.acces_banque_en_ligne = this.acces_banque_en_ligne;
	            other.alerte_securite_active = this.alerte_securite_active;
    ...
    public void readValuesData(DataInputStream dis, ObjectInputStream ois) {
        try {
			...
			int length = 0;
			            ...
			            this.id_digital_sk = dis.readInt();
						...
						this.acces_banque_en_ligne = readString(dis,ois);
    ...
    public void readValuesData(DataInputStream dis, org.jboss.marshalling.Unmarshaller objectIn) {
        try {
			int length = 0;
			            ...
			            this.id_digital_sk = objectIn.readInt();
					
			            length = objectIn.readByte();
           				if (length == -1) {
           	    			this.service_sms_banking = null;
           				} else {
           			    	this.service_sms_banking = objectIn.readLong();
           				}
					
						this.acces_banque_en_ligne = readString(dis,objectIn);
					
			            length = objectIn.readByte();
           				if (length == -1) {
           	    			this.alerte_securite_active = null;
           				} else {
           			    	this.alerte_securite_active = objectIn.readLong();
           				}
			            ...
			            length = objectIn.readByte();
           				if (length == -1) {
           			    	...
           			    	this.e_releve_active = objectIn.readLong();
           				}
    ...
    public void writeValuesData(DataOutputStream dos, ObjectOutputStream oos) {
        try {
		            	...
		            	dos.writeInt(this.id_digital_sk);
						...
						writeString(this.acces_banque_en_ligne, dos, oos);
    ...
    public void writeValuesData(DataOutputStream dos, org.jboss.marshalling.Marshaller objectOut){
                try {
					...
					objectOut.writeInt(this.id_digital_sk);
					
						if(this.service_sms_banking == null) {
							objectOut.writeByte(-1);
						} else {
							objectOut.writeByte(0);
							objectOut.writeLong(this.service_sms_banking);
		            	}
					
						writeString(this.acces_banque_en_ligne, dos, objectOut);
					
						if(this.alerte_securite_active == null) {
							objectOut.writeByte(-1);
						} else {
							objectOut.writeByte(0);
							objectOut.writeLong(this.alerte_securite_active);
		            	}
						...
						if(this.e_releve_active == null) {
							...
							objectOut.writeByte(0);
							objectOut.writeLong(this.e_releve_active);
		            	}
    ...
    public String toString() {
		...
		sb.append(super.toString());
		sb.append("[");
		sb.append("id_digital_sk="+String.valueOf(id_digital_sk));
		sb.append(",id_client="+id_client);
		sb.append(",service_sms_banking="+String.valueOf(service_sms_banking));
		sb.append(",acces_banque_en_ligne="+acces_banque_en_ligne);
		sb.append(",alerte_securite_active="+String.valueOf(alerte_securite_active));
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row2";
		...
		int tos_count_tAdvancedHash_row2 = 0;
			   		...
			   		// linked node: tMap_1 - inputs:(row1,row2,row3,row4,row5) outputs:(client)
 ...
 */
	...
	final String decryptedPassword_tDBInput_2 = context.password; 
		    ...
		    String dbquery_tDBInput_2 = "SELECT \n  \"DWH_ATTEJARI\".\"DIM_DIGITAL\".\"id_digital_sk\", \n  \"DWH_ATTEJARI\".\"DIM_DIGITAL\".\"id_client\", \n  \"D"
+"WH_ATTEJARI\".\"DIM_DIGITAL\".\"service_sms_banking\", \n  \"DWH_ATTEJARI\".\"DIM_DIGITAL\".\"acces_banque_en_ligne\", \n "
+" \"DWH_ATTEJARI\".\"DIM_DIGITAL\".\"alerte_securite_active\", \n  \"DWH_ATTEJARI\".\"DIM_DIGITAL\".\"e_releve_active\"\nFR"
+"OM \"DWH_ATTEJARI\".\"DIM_DIGITAL\"";
		    ...
		    while (rs_tDBInput_2.next()) {
		        nb_line_tDBInput_2++;
							...
							if(colQtyInRs_tDBInput_2 < 1) {
								row2.id_digital_sk = 0;
							} else {
		                          
            row2.id_digital_sk = rs_tDBInput_2.getInt(1);
        	...
        	row2.id_client = routines.system.JDBCUtil.getString(rs_tDBInput_2, 2, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 3) {
								row2.service_sms_banking = null;
							} else {
		                          
            row2.service_sms_banking = rs_tDBInput_2.getLong(3);
            if(rs_tDBInput_2.wasNull()){
                    row2.service_sms_banking = null;
            }
		                    }
							if(colQtyInRs_tDBInput_2 < 4) {
								row2.acces_banque_en_ligne = null;
							} else {
	                         		
        	row2.acces_banque_en_ligne = routines.system.JDBCUtil.getString(rs_tDBInput_2, 4, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 5) {
								row2.alerte_securite_active = null;
							} else {
		                          
            row2.alerte_securite_active = rs_tDBInput_2.getLong(5);
            if(rs_tDBInput_2.wasNull()){
                    row2.alerte_securite_active = null;
            }
		                    }
							if(colQtyInRs_tDBInput_2 < 6) {
								row2.e_releve_active = null;
							} else {
		                          
            row2.e_releve_active = rs_tDBInput_2.getLong(6);
            if(rs_tDBInput_2.wasNull()){
                    row2.e_releve_active = null;
            }
		                    }
 ...
 */
	...
	currentComponent="tAdvancedHash_row2";
				...
				row2_HashRow.id_digital_sk = row2.id_digital_sk;
				
				row2_HashRow.id_client = row2.id_client;
				
				row2_HashRow.service_sms_banking = row2.service_sms_banking;
				
				row2_HashRow.acces_banque_en_ligne = row2.acces_banque_en_ligne;
				
				row2_HashRow.alerte_securite_active = row2.alerte_securite_active;
				
				row2_HashRow.e_releve_active = row2.e_releve_active;
...
public static class row3Struct implements routines.system.IPersistableComparableLookupRow<row3Struct> {
    ...
    public void readValuesData(DataInputStream dis, ObjectInputStream ois) {
        try {
			...
			int length = 0;
						...
						this.methode_paiement_fav = readString(dis,ois);
    ...
    public void readValuesData(DataInputStream dis, org.jboss.marshalling.Unmarshaller objectIn) {
        try {
			int length = 0;
			            ...
			            this.id_financier_sk = objectIn.readInt();
					
			            this.id_type_convention_sk = objectIn.readInt();
					
						this.methode_paiement_fav = readString(dis,objectIn);
					
			            length = objectIn.readByte();
           				if (length == -1) {
           	    			this.frais_bancaires_mensuels = null;
           				} else {
           			    	this.frais_bancaires_mensuels = objectIn.readDouble();
           				}
			            ...
			            length = objectIn.readByte();
           				if (length == -1) {
           			    	...
           			    	this.total_frais_cumules = objectIn.readDouble();
           				}
			            ...
			            length = objectIn.readByte();
           				if (length == -1) {
           			    	...
           			    	this.revenu_mensuel = objectIn.readDouble();
           				}
    ...
    public void writeValuesData(DataOutputStream dos, ObjectOutputStream oos) {
        try {
						...
						writeString(this.methode_paiement_fav, dos, oos);
    ...
    public void writeValuesData(DataOutputStream dos, org.jboss.marshalling.Marshaller objectOut){
                try {
					...
					objectOut.writeInt(this.id_financier_sk);
					
					objectOut.writeInt(this.id_type_convention_sk);
					
						writeString(this.methode_paiement_fav, dos, objectOut);
					
						if(this.frais_bancaires_mensuels == null) {
							objectOut.writeByte(-1);
						} else {
							objectOut.writeByte(0);
							objectOut.writeDouble(this.frais_bancaires_mensuels);
		            	}
						...
						if(this.total_frais_cumules == null) {
							...
							objectOut.writeByte(0);
							objectOut.writeDouble(this.total_frais_cumules);
		            	}
						...
						if(this.revenu_mensuel == null) {
							...
							objectOut.writeByte(0);
							objectOut.writeDouble(this.revenu_mensuel);
		            	}
 ...
 */
	...
	final String decryptedPassword_tDBInput_3 = context.password; 
		    ...
		    String dbquery_tDBInput_3 = "SELECT \n  \""+context.schema+"\".\"DIM_FINANCIER\".\"id_financier_sk\", \n  \""+context.schema+"\".\"DIM_FINANCIER\".\"id_client\", \n  \""+context.schema+"\".\"DIM_FINANCIER\".\"id_type_convention_sk\", \n  \""+context.schema+"\".\"DIM_FINANCIER\".\"methode_paiement_fav\", \n  \""+context.schema+"\".\"DIM_FINANCIER\".\"frais_bancaires_mensuels\", \n  \""+context.schema+"\".\"DIM_FINANCIER\".\"total_frais_cumules\", \n  \""+context.schema+"\".\"DIM_FINANCIER\".\"revenu_mensuel\"\nFROM \""+context.schema+"\".\"DIM_FINANCIER\"";
		    ...
		    while (rs_tDBInput_3.next()) {
		        nb_line_tDBInput_3++;
							...
							if(colQtyInRs_tDBInput_3 < 1) {
								row3.id_financier_sk = 0;
							} else {
		                          
            row3.id_financier_sk = rs_tDBInput_3.getInt(1);
        	...
        	row3.id_client = routines.system.JDBCUtil.getString(rs_tDBInput_3, 2, false);
							...
							if(colQtyInRs_tDBInput_3 < 3) {
								row3.id_type_convention_sk = 0;
							} else {
		                          
            row3.id_type_convention_sk = rs_tDBInput_3.getInt(3);
            if(rs_tDBInput_3.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
							if(colQtyInRs_tDBInput_3 < 4) {
								row3.methode_paiement_fav = null;
							} else {
	                         		
        	row3.methode_paiement_fav = routines.system.JDBCUtil.getString(rs_tDBInput_3, 4, false);
		                    }
							if(colQtyInRs_tDBInput_3 < 5) {
								row3.frais_bancaires_mensuels = null;
							} else {
	                         		
            row3.frais_bancaires_mensuels = rs_tDBInput_3.getDouble(5);
            if(rs_tDBInput_3.wasNull()){
                    row3.frais_bancaires_mensuels = null;
            }
		                    }
							if(colQtyInRs_tDBInput_3 < 6) {
								row3.total_frais_cumules = null;
							} else {
	                         		
            row3.total_frais_cumules = rs_tDBInput_3.getDouble(6);
            if(rs_tDBInput_3.wasNull()){
                    row3.total_frais_cumules = null;
            }
		                    }
							if(colQtyInRs_tDBInput_3 < 7) {
								row3.revenu_mensuel = null;
							} else {
	                         		
            row3.revenu_mensuel = rs_tDBInput_3.getDouble(7);
            if(rs_tDBInput_3.wasNull()){
                    row3.revenu_mensuel = null;
            }
		                    }
 ...
 */
	...
	currentComponent="tAdvancedHash_row3";
				...
				row3_HashRow.id_financier_sk = row3.id_financier_sk;
				
				row3_HashRow.id_client = row3.id_client;
				
				row3_HashRow.id_type_convention_sk = row3.id_type_convention_sk;
				
				row3_HashRow.methode_paiement_fav = row3.methode_paiement_fav;
				
				row3_HashRow.frais_bancaires_mensuels = row3.frais_bancaires_mensuels;
				
				row3_HashRow.total_frais_cumules = row3.total_frais_cumules;
				
				row3_HashRow.revenu_mensuel = row3.revenu_mensuel;
...
public static class row4Struct implements routines.system.IPersistableComparableLookupRow<row4Struct> {
    ...
    public String loopKey;
			    ...
			    public int id_produit_sk;

				public int getId_produit_sk () {
					return this.id_produit_sk;
				}
	...
	public void copyDataTo(row4Struct other) {
		...
		other.id_produit_sk = this.id_produit_sk;
	            other.id_client = this.id_client;
	            other.nb_produits_actifs = this.nb_produits_actifs;
	            other.conseiller_dedie = this.conseiller_dedie;
	            other.programme_fidelite = this.programme_fidelite;
    ...
    public void readValuesData(DataInputStream dis, ObjectInputStream ois) {
        try {
			...
			int length = 0;
			            ...
			            this.id_produit_sk = dis.readInt();
    ...
    public void readValuesData(DataInputStream dis, org.jboss.marshalling.Unmarshaller objectIn) {
        try {
			int length = 0;
			            ...
			            this.id_produit_sk = objectIn.readInt();
					
			            length = objectIn.readByte();
           				if (length == -1) {
           	    			this.nb_produits_actifs = null;
           				} else {
           			    	this.nb_produits_actifs = objectIn.readLong();
           				}
			            ...
			            length = objectIn.readByte();
           				if (length == -1) {
           			    	...
           			    	this.conseiller_dedie = objectIn.readLong();
           				}
			            ...
			            length = objectIn.readByte();
           				if (length == -1) {
           			    	...
           			    	this.programme_fidelite = objectIn.readLong();
           				}
			            ...
			            length = objectIn.readByte();
           				if (length == -1) {
           			    	...
           			    	this.assurance_moyen_paiement = objectIn.readLong();
           				}
    ...
    public void writeValuesData(DataOutputStream dos, ObjectOutputStream oos) {
        try {
		            	...
		            	dos.writeInt(this.id_produit_sk);
    ...
    public void writeValuesData(DataOutputStream dos, org.jboss.marshalling.Marshaller objectOut){
                try {
					...
					objectOut.writeInt(this.id_produit_sk);
					
						if(this.nb_produits_actifs == null) {
							objectOut.writeByte(-1);
						} else {
							objectOut.writeByte(0);
							objectOut.writeLong(this.nb_produits_actifs);
		            	}
						...
						if(this.conseiller_dedie == null) {
							...
							objectOut.writeByte(0);
							objectOut.writeLong(this.conseiller_dedie);
		            	}
						...
						if(this.programme_fidelite == null) {
							...
							objectOut.writeByte(0);
							objectOut.writeLong(this.programme_fidelite);
		            	}
						...
						if(this.assurance_moyen_paiement == null) {
							...
							objectOut.writeByte(0);
							objectOut.writeLong(this.assurance_moyen_paiement);
		            	}
    ...
    public String toString() {
		...
		sb.append(super.toString());
		sb.append("[");
		sb.append("id_produit_sk="+String.valueOf(id_produit_sk));
		sb.append(",id_client="+id_client);
		sb.append(",nb_produits_actifs="+String.valueOf(nb_produits_actifs));
		sb.append(",conseiller_dedie="+String.valueOf(conseiller_dedie));
		sb.append(",programme_fidelite="+String.valueOf(programme_fidelite));
 ...
 */
	...
	final String decryptedPassword_tDBInput_4 = context.password; 
		    ...
		    String dbquery_tDBInput_4 = "SELECT \n  \"DWH_ATTEJARI\".\"DIM_PRODUIT\".\"id_produit_sk\", \n  \"DWH_ATTEJARI\".\"DIM_PRODUIT\".\"id_client\", \n  \"D"
+"WH_ATTEJARI\".\"DIM_PRODUIT\".\"nb_produits_actifs\", \n  \"DWH_ATTEJARI\".\"DIM_PRODUIT\".\"conseiller_dedie\", \n  \"DWH"
+"_ATTEJARI\".\"DIM_PRODUIT\".\"programme_fidelite\", \n  \"DWH_ATTEJARI\".\"DIM_PRODUIT\".\"assurance_moyen_paiement\"\nFRO"
+"M \"DWH_ATTEJARI\".\"DIM_PRODUIT\"";
		    ...
		    while (rs_tDBInput_4.next()) {
		        nb_line_tDBInput_4++;
							...
							if(colQtyInRs_tDBInput_4 < 1) {
								row4.id_produit_sk = 0;
							} else {
		                          
            row4.id_produit_sk = rs_tDBInput_4.getInt(1);
        	...
        	row4.id_client = routines.system.JDBCUtil.getString(rs_tDBInput_4, 2, false);
							...
							if(colQtyInRs_tDBInput_4 < 3) {
								row4.nb_produits_actifs = null;
							} else {
		                          
            row4.nb_produits_actifs = rs_tDBInput_4.getLong(3);
            if(rs_tDBInput_4.wasNull()){
                    row4.nb_produits_actifs = null;
            }
		                    }
							if(colQtyInRs_tDBInput_4 < 4) {
								row4.conseiller_dedie = null;
							} else {
		                          
            row4.conseiller_dedie = rs_tDBInput_4.getLong(4);
            if(rs_tDBInput_4.wasNull()){
                    row4.conseiller_dedie = null;
            }
		                    }
							if(colQtyInRs_tDBInput_4 < 5) {
								row4.programme_fidelite = null;
							} else {
		                          
            row4.programme_fidelite = rs_tDBInput_4.getLong(5);
            if(rs_tDBInput_4.wasNull()){
                    row4.programme_fidelite = null;
            }
		                    }
							if(colQtyInRs_tDBInput_4 < 6) {
								row4.assurance_moyen_paiement = null;
							} else {
		                          
            row4.assurance_moyen_paiement = rs_tDBInput_4.getLong(6);
            if(rs_tDBInput_4.wasNull()){
                    row4.assurance_moyen_paiement = null;
            }
		                    }
 ...
 */
	...
	currentComponent="tAdvancedHash_row4";
				...
				row4_HashRow.id_produit_sk = row4.id_produit_sk;
				
				row4_HashRow.id_client = row4.id_client;
				
				row4_HashRow.nb_produits_actifs = row4.nb_produits_actifs;
				
				row4_HashRow.conseiller_dedie = row4.conseiller_dedie;
				
				row4_HashRow.programme_fidelite = row4.programme_fidelite;
				
				row4_HashRow.assurance_moyen_paiement = row4.assurance_moyen_paiement;
...
public static class row5Struct implements routines.system.IPersistableComparableLookupRow<row5Struct> {
    ...
    public String loopKey;
			    ...
			    public int id_profil_sk;

				public int getId_profil_sk () {
					return this.id_profil_sk;
				}
	...
	public void copyDataTo(row5Struct other) {
		...
		other.id_profil_sk = this.id_profil_sk;
	            other.id_client = this.id_client;
	            other.situation_matrimoniale = this.situation_matrimoniale;
	            other.enfants_a_charge = this.enfants_a_charge;
	            other.profession = this.profession;
    ...
    public void readValuesData(DataInputStream dis, ObjectInputStream ois) {
        try {
			...
			int length = 0;
						...
						this.ville_region = readString(dis,ois);
    ...
    public void readValuesData(DataInputStream dis, org.jboss.marshalling.Unmarshaller objectIn) {
        try {
			int length = 0;
			            ...
			            this.id_profil_sk = objectIn.readInt();
			            ...
			            length = objectIn.readByte();
           				if (length == -1) {
           			    	...
           			    	this.enfants_a_charge = objectIn.readLong();
           				}
					
						this.profession = readString(dis,objectIn);
					
						this.ville_region = readString(dis,objectIn);
					
			            length = objectIn.readByte();
           				if (length == -1) {
           	    			this.est_senior = null;
           				} else {
           			    	this.est_senior = objectIn.readLong();
           				}
    ...
    public void writeValuesData(DataOutputStream dos, ObjectOutputStream oos) {
        try {
		            	...
		            	dos.writeInt(this.id_profil_sk);
						...
						writeString(this.ville_region, dos, oos);
    ...
    public void writeValuesData(DataOutputStream dos, org.jboss.marshalling.Marshaller objectOut){
                try {
					...
					objectOut.writeInt(this.id_profil_sk);
						...
						if(this.enfants_a_charge == null) {
							...
							objectOut.writeByte(0);
							objectOut.writeLong(this.enfants_a_charge);
		            	}
					
						writeString(this.profession, dos, objectOut);
					
						writeString(this.ville_region, dos, objectOut);
					
						if(this.est_senior == null) {
							objectOut.writeByte(-1);
						} else {
							objectOut.writeByte(0);
							objectOut.writeLong(this.est_senior);
		            	}
    ...
    public String toString() {
		...
		sb.append(super.toString());
		sb.append("[");
		sb.append("id_profil_sk="+String.valueOf(id_profil_sk));
		sb.append(",id_client="+id_client);
		sb.append(",situation_matrimoniale="+situation_matrimoniale);
		sb.append(",enfants_a_charge="+String.valueOf(enfants_a_charge));
		sb.append(",profession="+profession);
 ...
 */
	...
	final String decryptedPassword_tDBInput_5 = context.password; 
		    ...
		    String dbquery_tDBInput_5 = "SELECT \n  \"DWH_ATTEJARI\".\"DIM_PROFIL\".\"id_profil_sk\", \n  \"DWH_ATTEJARI\".\"DIM_PROFIL\".\"id_client\", \n  \"DWH_"
+"ATTEJARI\".\"DIM_PROFIL\".\"situation_matrimoniale\", \n  \"DWH_ATTEJARI\".\"DIM_PROFIL\".\"enfants_a_charge\", \n  \"DWH_"
+"ATTEJARI\".\"DIM_PROFIL\".\"profession\", \n  \"DWH_ATTEJARI\".\"DIM_PROFIL\".\"ville_region\", \n  \"DWH_ATTEJARI\".\"DIM"
+"_PROFIL\".\"est_senior\"\nFROM \"DWH_ATTEJARI\".\"DIM_PROFIL\"";
            ...
            row5.id_profil_sk = rs_tDBInput_5.getInt(1);
        	...
        	row5.id_client = routines.system.JDBCUtil.getString(rs_tDBInput_5, 2, false);
		                    }
							if(colQtyInRs_tDBInput_5 < 3) {
								row5.situation_matrimoniale = null;
							} else {
	                         		
        	row5.situation_matrimoniale = routines.system.JDBCUtil.getString(rs_tDBInput_5, 3, false);
		                    }
							if(colQtyInRs_tDBInput_5 < 4) {
								row5.enfants_a_charge = null;
							} else {
		                          
            row5.enfants_a_charge = rs_tDBInput_5.getLong(4);
            if(rs_tDBInput_5.wasNull()){
                    row5.enfants_a_charge = null;
            }
		                    }
        	...
        	row5.profession = routines.system.JDBCUtil.getString(rs_tDBInput_5, 5, false);
		                    }
							if(colQtyInRs_tDBInput_5 < 6) {
								row5.ville_region = null;
							} else {
	                         		
        	row5.ville_region = routines.system.JDBCUtil.getString(rs_tDBInput_5, 6, false);
		                    }
							if(colQtyInRs_tDBInput_5 < 7) {
								row5.est_senior = null;
							} else {
		                          
            row5.est_senior = rs_tDBInput_5.getLong(7);
            if(rs_tDBInput_5.wasNull()){
                    row5.est_senior = null;
            }
		                    }
 ...
 */
	...
	currentComponent="tAdvancedHash_row5";
				...
				row5_HashRow.id_profil_sk = row5.id_profil_sk;
				
				row5_HashRow.id_client = row5.id_client;
				
				row5_HashRow.situation_matrimoniale = row5.situation_matrimoniale;
				
				row5_HashRow.enfants_a_charge = row5.enfants_a_charge;
				
				row5_HashRow.profession = row5.profession;
				
				row5_HashRow.ville_region = row5.ville_region;
				
				row5_HashRow.est_senior = row5.est_senior;
 ...
 */
    ...
    public static void main(String[] args){
        final J_Load_DIM_CLIENT J_Load_DIM_CLIENTClass = new J_Load_DIM_CLIENT();

        int exitCode = J_Load_DIM_CLIENTClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = J_Load_DIM_CLIENT.class.getClassLoader().getResourceAsStream("dwh_attejaribank/j_load_dim_client_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = J_Load_DIM_CLIENT.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : J_Load_DIM_CLIENT");
        }
...
/************************************************************************************************
 *     230189 characters generated by Talend Open Studio for Data Integration 
 *     on the 16 avril 2026 à 15:48:59 WAT
 ************************************************************************************************/