...
package dwh_attejaribank.job_load_dim_statut_recl_0_1;
...
/**
 * Job: Job_Load_DIM_STATUT_RECL Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class Job_Load_DIM_STATUT_RECL implements TalendJob {
...
public String getSchema_staging(){
	...
	private final String jobName = "Job_Load_DIM_STATUT_RECL";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Job_Load_DIM_STATUT_RECL.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Job_Load_DIM_STATUT_RECL.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
    final static byte[] commonByteArrayLock_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL = new byte[0];
    static byte[] commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL = new byte[0];
    ...
    public String loopKey;
			    ...
			    public int id_status_sk;

				public int getId_status_sk () {
					return this.id_status_sk;
				}
	...
	public void copyDataTo(row3Struct other) {
		...
		other.id_status_sk = this.id_status_sk;
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL.length) {
				if(length < 1024 && commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL.length == 0) {
   					commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL = new byte[1024];
				} else {
   					commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL, 0, length);
			strReturn = new String(commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL, 0, length);
			strReturn = new String(commonByteArray_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_DWH_ATTEJARIBANK_Job_Load_DIM_STATUT_RECL) {

        	try {

        		int length = 0;
		
			        this.id_status_sk = dis.readInt();
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// int
		            	...
		            	dos.writeInt(this.id_status_sk);
    ...
    public String toString() {
		...
		sb.append("[");
		sb.append("id_status_sk="+String.valueOf(id_status_sk));
...
if(dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
	tableName_tDBOutput_1 = ("DIM_STATUS_Reclamation");
} else {
	tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("DIM_STATUS_Reclamation");
}
...
int count_tDBOutput_1=0;
                                ...
                                try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null, new String[]{"TABLE"})) {
                                    ...
                                    while(rsTable_tDBOutput_1.next()) {
                                        ...
                                        if(table_tDBOutput_1.equals(("DIM_STATUS_Reclamation"))
                                ...
                                if(!whetherExist_tDBOutput_1) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
                                        stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1 + "\"(\"id_status_sk\" SERIAL  not null ,\"statut_resolution\" VARCHAR(14)  ,primary key(\"statut_resolution\"))");
                                    }
                                }
	    String update_tDBOutput_1 = "UPDATE \"" + tableName_tDBOutput_1 + "\" SET \"statut_resolution\" = ? WHERE \"statut_resolution\" = ?";
	    java.sql.PreparedStatement pstmtUpdate_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(update_tDBOutput_1);
	    resourceMap.put("pstmtUpdate_tDBOutput_1", pstmtUpdate_tDBOutput_1);
	    String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1 + "\" (\"statut_resolution\") VALUES (?)";
...
// # Output table : 'status'
status_tmp.id_status_sk = 0;
status_tmp.statut_resolution = row1.statut_resolution ;
status = status_tmp;
// ###############################
...
row2 = null;			
if(status.statut_resolution == null){
	finder_tUniqRow_1.statut_resolution = null;
}else{
	finder_tUniqRow_1.statut_resolution = status.statut_resolution.toLowerCase();
}	
...
if(status.statut_resolution == null){
	new_tUniqRow_1.statut_resolution = null;
}else{
	new_tUniqRow_1.statut_resolution = status.statut_resolution.toLowerCase();
}
		
		keystUniqRow_1.add(new_tUniqRow_1);if(row2 == null){ 
	
	row2 = new row2Struct();
}row2.id_status_sk = status.id_status_sk;			row2.statut_resolution = status.statut_resolution;					
...
///////////////////////		
				...
				strBuffer_tLogRow_1.append(
				                String.valueOf(row2.id_status_sk)							
				);
	    		...
	    		if(row2.statut_resolution != null) { //              
                    							
       
				strBuffer_tLogRow_1.append(
				                String.valueOf(row2.statut_resolution)							
				);
	...
	currentComponent="tDBOutput_1";
        ...
        whetherReject_tDBOutput_1 = false;
            int updateFlag_tDBOutput_1=0;
                    if(row3.statut_resolution == null) {
pstmtUpdate_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(1, row3.statut_resolution);
}


                    if(row3.statut_resolution == null) {
pstmtUpdate_tDBOutput_1.setNull(2 + count_tDBOutput_1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(2 + count_tDBOutput_1, row3.statut_resolution);
}
...
} else {pstmtInsert_tDBOutput_1.setString(1, row3.statut_resolution);
}
 ...
 */
    ...
    public static void main(String[] args){
        final Job_Load_DIM_STATUT_RECL Job_Load_DIM_STATUT_RECLClass = new Job_Load_DIM_STATUT_RECL();

        int exitCode = Job_Load_DIM_STATUT_RECLClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Job_Load_DIM_STATUT_RECL.class.getClassLoader().getResourceAsStream("dwh_attejaribank/job_load_dim_statut_recl_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Job_Load_DIM_STATUT_RECL.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Job_Load_DIM_STATUT_RECL");
        }
...
/************************************************************************************************
 *     95876 characters generated by Talend Open Studio for Data Integration 
