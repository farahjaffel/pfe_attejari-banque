...
package dwh_attejaribank.j_load_dim_produit_0_1;
...
/**
 * Job: J_Load_DIM_Produit Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class J_Load_DIM_Produit implements TalendJob {
...
public String getSchema_staging(){
	...
	private final String jobName = "J_Load_DIM_Produit";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				J_Load_DIM_Produit.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(J_Load_DIM_Produit.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class produitStruct implements routines.system.IPersistableRow<produitStruct> {
    final static byte[] commonByteArrayLock_DWH_ATTEJARIBANK_J_Load_DIM_Produit = new byte[0];
    static byte[] commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit = new byte[0];
    ...
    public String loopKey;
			    ...
			    public Integer id_produit_sk;

				public Integer getId_produit_sk () {
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit.length) {
				if(length < 1024 && commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit.length == 0) {
   					commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit = new byte[1024];
				} else {
   					commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit, 0, length);
			strReturn = new String(commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit, 0, length);
			strReturn = new String(commonByteArray_DWH_ATTEJARIBANK_J_Load_DIM_Produit, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_DWH_ATTEJARIBANK_J_Load_DIM_Produit) {

        	try {

        		int length = 0;
		
						this.id_produit_sk = readInteger(dis);
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// Integer
						...
						writeInteger(this.id_produit_sk,dos);
...
int count_tDBOutput_1=0;
                                ...
                                if(!whetherExist_tDBOutput_1) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
                                        stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1 + "\"(\"id_produit_sk\" SERIAL ,\"id_client\" VARCHAR(10)   not null ,\"nb_produits_actifs\" INT8 ,\"conseiller_dedie\" INT8 ,\"programme_fidelite\" INT8 ,\"assurance_moyen_paiement\" INT8 ,primary key(\"id_client\"))");
                                    }
                                }
	    String update_tDBOutput_1 = "UPDATE \"" + tableName_tDBOutput_1 + "\" SET \"id_client\" = ?,\"nb_produits_actifs\" = ?,\"conseiller_dedie\" = ?,\"programme_fidelite\" = ?,\"assurance_moyen_paiement\" = ? WHERE \"id_client\" = ?";
	    java.sql.PreparedStatement pstmtUpdate_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(update_tDBOutput_1);
	    resourceMap.put("pstmtUpdate_tDBOutput_1", pstmtUpdate_tDBOutput_1);
	    String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1 + "\" (\"id_client\",\"nb_produits_actifs\",\"conseiller_dedie\",\"programme_fidelite\",\"assurance_moyen_paiement\") VALUES (?,?,?,?,?)";
...
produit_tmp.id_produit_sk = null;
produit_tmp.id_client = row1.id_client ;
produit_tmp.nb_produits_actifs = row1.nb_produits_actifs ;
produit_tmp.conseiller_dedie = row1.conseiller_dedie ;
produit_tmp.programme_fidelite = row1.programme_fidelite ;
produit_tmp.assurance_moyen_paiement = row1.assurance_moyen_paiement ;
produit = produit_tmp;
// ###############################
	...
	currentComponent="tDBOutput_1";
        ...
        whetherReject_tDBOutput_1 = false;
            int updateFlag_tDBOutput_1=0;
                    if(produit.id_client == null) {
pstmtUpdate_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(1, produit.id_client);
}

                    if(produit.nb_produits_actifs == null) {
pstmtUpdate_tDBOutput_1.setNull(2, java.sql.Types.INTEGER);
} else {pstmtUpdate_tDBOutput_1.setLong(2, produit.nb_produits_actifs);
}

                    if(produit.conseiller_dedie == null) {
pstmtUpdate_tDBOutput_1.setNull(3, java.sql.Types.INTEGER);
} else {pstmtUpdate_tDBOutput_1.setLong(3, produit.conseiller_dedie);
}

                    if(produit.programme_fidelite == null) {
pstmtUpdate_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
} else {pstmtUpdate_tDBOutput_1.setLong(4, produit.programme_fidelite);
}

                    if(produit.assurance_moyen_paiement == null) {
pstmtUpdate_tDBOutput_1.setNull(5, java.sql.Types.INTEGER);
} else {pstmtUpdate_tDBOutput_1.setLong(5, produit.assurance_moyen_paiement);
}


                    if(produit.id_client == null) {
pstmtUpdate_tDBOutput_1.setNull(6 + count_tDBOutput_1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(6 + count_tDBOutput_1, produit.id_client);
}
...
} else {pstmtInsert_tDBOutput_1.setString(1, produit.id_client);
}

                        if(produit.nb_produits_actifs == null) {
pstmtInsert_tDBOutput_1.setNull(2, java.sql.Types.INTEGER);
} else {pstmtInsert_tDBOutput_1.setLong(2, produit.nb_produits_actifs);
}

                        if(produit.conseiller_dedie == null) {
pstmtInsert_tDBOutput_1.setNull(3, java.sql.Types.INTEGER);
} else {pstmtInsert_tDBOutput_1.setLong(3, produit.conseiller_dedie);
}

                        if(produit.programme_fidelite == null) {
pstmtInsert_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
} else {pstmtInsert_tDBOutput_1.setLong(4, produit.programme_fidelite);
}

                        if(produit.assurance_moyen_paiement == null) {
pstmtInsert_tDBOutput_1.setNull(5, java.sql.Types.INTEGER);
} else {pstmtInsert_tDBOutput_1.setLong(5, produit.assurance_moyen_paiement);
}
 ...
 */
    ...
    public static void main(String[] args){
        final J_Load_DIM_Produit J_Load_DIM_ProduitClass = new J_Load_DIM_Produit();

        int exitCode = J_Load_DIM_ProduitClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = J_Load_DIM_Produit.class.getClassLoader().getResourceAsStream("dwh_attejaribank/j_load_dim_produit_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = J_Load_DIM_Produit.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : J_Load_DIM_Produit");
        }
...
/************************************************************************************************
 *     96978 characters generated by Talend Open Studio for Data Integration 
 *     on the 16 avril 2026 à 15:48:55 WAT
 ************************************************************************************************/