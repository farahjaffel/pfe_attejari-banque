...
Améliorations v2.1 :
  - autocommit=True : fix du problème d'insertion invisible dans pgAdmin
...
Auteur: Farah Jaffel | Version Kafka 2.1
"""
...
FLUSH_INTERVAL = 5     # OU forcer l'insertion toutes les 5 secondes
...
# autocommit=True → pas besoin de conn.commit()
# ======================================================================
...
def consumer_clients():
    ...
    try:
        ...
        conn.autocommit = True   # ← FIX : chaque INSERT est immédiatement visible
        cur = conn.cursor()
        log(topic, "✅ Connexion PostgreSQL OK (autocommit=True)")
    except Exception as e:
...
if __name__ == "__main__":
    print("\n" + "=" * 65)
    print("  CONSUMER KAFKA v2.1 - BANQUE VIRTUELLE TUNISIENNE")
    print(f"  Batch size : {BATCH_SIZE} msg  |  Flush toutes les {FLUSH_INTERVAL}s")
    print("  Mode : autocommit=True (insertions immédiatement visibles)")
    print("=" * 65 + "\n")
